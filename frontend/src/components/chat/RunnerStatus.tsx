...
function formatRelative(value: string | number | null | undefined, t: (key: string, vars?: Record<string, string | number>) => string): string {
  ...
  if (deltaSec < 0) return t("just now");
  if (deltaSec < 60) return t("{count}s ago", { count: deltaSec });
  if (deltaSec < 3600) return t("{count}m ago", { count: Math.floor(deltaSec / 60) });
  if (deltaSec < 86_400) return t("{count}h ago", { count: Math.floor(deltaSec / 3600) });
  return t("{count}d ago", { count: Math.floor(deltaSec / 86_400) });
}

function formatCountdown(iso: string | undefined, t: (key: string) => string): { label: string; expired: boolean; soon: boolean } {
  if (!iso) return { label: "—", expired: false, soon: false };
  const target = new Date(iso).getTime();
  if (!Number.isFinite(target)) return { label: t("unknown"), expired: false, soon: false };
  const deltaSec = Math.round((target - Date.now()) / 1000);
  if (deltaSec <= 0) return { label: t("expired"), expired: true, soon: false };
...
function BrokerRow({
  ...
  const authorizeInstruction = authorizeHint?.instruction
    ?? (authorizeFailed ? t("Run `vibe-trading connector list`, choose the broker profile, then run `vibe-trading connector authorize <profile>` from the desktop session that will hold the broker connection.") : t("Loading connector authorization instructions..."));
  const authorizeNote = t("The connector channel stays read-only until OAuth succeeds and a mandate is committed.");
  ...
  const toggleRunner = useCallback(async () => {
    ...
    try {
      if (runnerAlive) {
        ...
        toast.success(t("Runner stopped for {broker}", { broker: brokerKey }));
      } else {
        await api.startLiveRunner(brokerKey);
        toast.success(t("Runner started for {broker}", { broker: brokerKey }));
      }
      onRefresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("Runner control failed."));
    } finally {
      setBusy(false);
    }
  }, [brokerKey, busy, runnerAlive, onRefresh, t]);
      ...
      {!authorized ? (
        <div className="grid gap-1.5 rounded-md border border-dashed border-primary/30 bg-primary/5 p-2">
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-primary">
            ...
            {t("Connect this profile to enable connector runtime")}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-md border bg-background/60 p-2">
              ...
              <div className={["mt-0.5 text-xs font-semibold", runnerAlive ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"].join(" ")}>
                {runnerAlive ? t("Running") : t("Stopped")}
              </div>
            </div>
            <div className="rounded-md border bg-background/60 p-2">
              <div className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {mandate.expires_at && (
                  <span
                    ...
                    title={`${t("expires {date}", { date: new Date(mandate.expires_at).toLocaleString() })}`}
                  >
                    <Clock className="h-2.5 w-2.5" />
                    {countdown.expired ? t("expired") : t("expires in {time}", { time: countdown.label })}
                  </span>
                )}
              </div>
              <div className="mt-0.5 font-mono text-[11px] text-foreground">
                {summarizeLimits(mandate.limits) || t("limits unavailable")}
              </div>
            </div>
          ) : (
            <div className="rounded-md border border-dashed bg-background/40 p-2 text-[10px] text-muted-foreground">
              {t("No active mandate. Ask the agent to propose one, then commit it before starting the connector runtime.")}
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-2">
            {halted ? (
              <span className="inline-flex items-center gap-1 text-[10px] font-medium text-destructive">
                ...
                {t("Halted — runner controls disabled")}
              </span>
            ) : (
              <span className="text-[10px] text-muted-foreground">
                {runnerAlive ? t("Runtime active inside mandate") : t("Idle")}
              </span>
            )}
            <button
              type="button"
              ...
              title={runnerAlive ? t("Stop the persistent runner") : t("Start the persistent runner")}
            >
              {busy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Power className="h-3 w-3" />}
              {runnerAlive ? t("Stop runner") : t("Start runner")}
            </button>
          </div>
        </>
      )}
  ...
  return (
    <div className="grid gap-2">
      <button
        ...
        aria-label={t("Connector runtime status")}
        aria-expanded={open}
      >
        <Activity className="h-3 w-3 shrink-0" />
        <span className="shrink-0">{t("Connector runtime")}</span>
        <span className="truncate text-muted-foreground">
          {authorizedCount > 0 ? t("{count} connected", { count: authorizedCount }) : t("no connector connected")}
        </span>
        {anyRunning && !isHalted && (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600 dark:text-emerald-400">
