...
export function Correlation() {
  ...
  const compute = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("Failed to compute correlation"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold">{t("Correlation Matrix")}</h1>
      </div>

      {/* Controls */}
      <div className="flex flex-col gap-4 border rounded-lg p-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium">{t("Asset codes")}</label>
          <input
            type="text"
            value={codes}
          ...
          <p className="text-xs text-muted-foreground">
            {t("Comma-separated ticker symbols, e.g. BTC-USDT,ETH-USDT,AAPL,SPY")}
          </p>
        </div>

        <div className="flex flex-wrap gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">{t("Window (days)")}</label>
