...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(t("Failed to load LLM settings: {message}", { message }));
          toast.error(t("Failed to load data source settings: {message}", { message }));
        }
      })
      .finally(() => {
  ...
  const submitLocalApiKey = (event: FormEvent) => {
    ...
    toast.success(t("Local API key saved"));
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setApiKey("");
      setClearApiKey(false);
      toast.success(t("LLM settings saved"));
    } catch (error) {
      toast.error(t("Failed to save LLM settings: {message}", { message: error instanceof Error ? error.message : t("Unknown error") }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success(t("Data source settings saved"));
    } catch (error) {
      toast.error(t("Failed to save data source settings: {message}", { message: error instanceof Error ? error.message : t("Unknown error") }));
    } finally {
      setDataSaving(false);
    }
  };
    ...
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold">{t("Local API access")}</h2>
        </div>
        <p className="text-sm text-muted-foreground">{t("For remote or private Web UI deployments, enter the server API key once in this browser. Localhost use can stay blank.")}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          <span className={labelClass}>{t("Server API key")}</span>
          <input
            type="password"
            value={localApiKey}
            onChange={(event) => setLocalApiKeyState(event.target.value)}
            className={fieldClass}
            placeholder={t("Stored only in this browser. Leave blank to clear it.")}
      ...
      <p className="mt-2 text-xs text-muted-foreground">{t("Stored only in this browser. Leave blank to clear it.")}</p>
    </form>
  );
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{t("Configure model credentials and market data source tokens for this local project.")}</p>
        </div>
        ...
        <div className="flex min-h-32 items-center justify-center rounded-lg border bg-card p-5 text-sm text-muted-foreground">
          {settingsLoadError ? (
            <div className="text-center">
              <div className="font-medium text-foreground">{t("Settings are unavailable")}</div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? t("This provider uses OAuth. Run: {command}", { command: selectedProvider.login_command })
        : t("This provider does not require an API key.");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        <h2 className="text-lg font-semibold tracking-tight">{t("LLM Settings")}</h2>
        <p className="max-w-3xl text-sm text-muted-foreground">{t("Choose the model used by the agent and save it to the project-local agent/.env file.")}</p>
      </div>
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("Connection")}</h2>
          </div>
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{t("Changing providers updates the recommended model and endpoint.")}</span>
            </label>
            ...
            <label className="grid gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  title={t("Use provider defaults")}
                >
                  <RotateCcw className="h-4 w-4" />
                  <span className="hidden sm:inline">{t("Use provider defaults")}</span>
                </button>
              </div>
              <span className={hintClass}>{t("Use the exact model id required by your provider.")}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>{t("Base URL")}</span>
              <input
                value={form.base_url}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>
                {selectedProvider?.auth_type === "oauth" ? t("OAuth") : t("API key")}
              </span>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("Generation")}</h2>
          </div>

          <div className="grid gap-4">
            <label className="grid gap-2">
              <span className={labelClass}>{t("Temperature")}</span>
              <input
                type="number"
                min={0}
                max={2}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("Timeout seconds")}</span>
              <input
                type="number"
                min={1}
                max={3600}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("Max retries")}</span>
              <input
                type="number"
                min={0}
                max={20}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("Reasoning effort")}</span>
              <select
              ...
              <span className={hintClass}>{t("How hard the model thinks before answering. Higher is more thorough but slower; leave Off for fastest replies.")}</span>
            </label>

            <div className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{t("Saved to")}: </span>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("Data Source Settings")}</h2>
          </div>
          <p className="text-sm text-muted-foreground">{t("Configure optional market data credentials used by backtests and research agents.")}</p>
        </div>

        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              <span className={labelClass}>{t("Tushare token")}</span>
              <div className="relative">
                <KeyRound className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
                <input
                  type="password"
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{t("Used for China A-share, futures, fund, and macro data. If unset, the project falls back to AKShare where available.")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
            ...
            >
              ...
              {dataSaving ? t("Saving...") : t("Save data source settings")}
            </button>
          </div>
          ...
          <div className="rounded-md border bg-muted/20 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs ${dataSettings.baostock_supported ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                {dataSettings.baostock_supported ? t("Loader available") : t("No project loader")}
              </span>
            </div>
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p>
                {dataSettings.baostock_installed
                  ? t("Python package installed")
                  : t("Python package not installed")}
              </p>
            </div>
          </div>
        </div>
